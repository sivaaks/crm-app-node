...
const contactsService={
    ...
    async getRequests(req,res){
        try{
            const data= await db.service_requests.find({}).project({requirement:1,description:1,status:1,contact:1}).toArray();
            res.send(data);
        }catch(err){
    ...
    async updateRequest(req,res){
        try{
            const {value:updatedValue}= await db.service_requests.findOneAndUpdate({_id:ObjectId(req.params.id)},{$set:{...req.body}},{returnDocument:"after"});
