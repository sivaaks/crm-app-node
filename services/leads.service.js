...
const contactsService={
    ...
    async getLeads(req,res){
        try{
            const data= await db.leads.find({},{requirement:1,description:1,status:1,contact:1}).toArray();
            res.send(data);
        }catch(err){
    ...
    async updateLead(req,res){
        try{
            const {value:updatedValue}= await db.leads.findOneAndUpdate({_id:ObjectId(req.params.id)},{$set:{...req.body}},{returnDocument:"after"});
