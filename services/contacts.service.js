...
const contactsService={
    ...
    async insertContact(req,res){
        ...
        const isContactExists= await db.contacts.findOne({email:req.body.email},{phone:req.body.phone});
    ...
    async getContacts(req,res){
        try{
            const data= await db.contacts.find({},{name:1,email:1,phone:1}).toArray();
            res.send(data);
        }catch(err){
    ...
    async updateContact(req,res){
        try{
            const {value:updatedValue}= await db.contacts.findOneAndUpdate({_id:ObjectId(req.params.id)},{$set:{...req.body}},{returnDocument:"after"});
