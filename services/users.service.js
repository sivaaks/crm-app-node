...
const service={
    ...
    async getUsers(req,res){
        try{
            const data= await db.users.find({}).project({name:1,email:1,userType:1}).toArray();
            res.status(200).send(data);
        }catch(err){
    ...
    async updateUser(req,res){
        try{
            const {value:updatedValue}= await db.users.findOneAndUpdate({_id:ObjectId(req.params.id)},{$set:{...req.body}},{returnDocument:"after"});
    ...
    async loginUser(req,res){
        try{
            ...
            const {verified}= await db.users.findOne({email:user.email},{verified:1});
    ...
    async forgotPassword(req,res){
        try{
            ...
            await db.users.findOneAndUpdate({email:user.email},{$set:{resetToken}});
            const link=`http://localhost:3001/users/reset-password/${resetToken}`;
            await sendEmail(user.email,'Forgot password','',`<p>Click on the below link to reset your password</p> <br> <a href=${link}>${link}</a>`);
            res.status(200).send(`Check your email (${user.email}) ${link} for password reset link`);
        }catch(err){
    ...
    async resetPassword(req,res){
        try{
            const user= await db.users.findOne({email:req.body.email,resetToken:req.params.id});
            if(user.resetToken===req.params.id) {
                try{
                    db.users.findOneAndUpdate({email:req.body.email},{$unset:{resetToken:1}});
                    const passwordEncrypt= await service.encryptPassword(req.body.password);
                    db.users.findOneAndUpdate({email:req.body.email},{$set:{password:passwordEncrypt}});
    ...
    async verifyUser(req,res){
        try {
                ...
                if(verify.verifyToken==req.query.token) {
                    const user= await db.users.findOneAndUpdate({verifyToken:req.query.token},{$unset:{verifyToken:1}});
                    db.users.findOneAndUpdate({email:user.value.email},{$set:{verified:true}});
                    res.status(200).send('Email verified successfully');
                }
            } catch (err) {
               res.status(400).send('Email cannot be verified or link has expired. Try again later');
            }
        }
